import type {
  ...
  Course, CourseDetail, Enrollment, User,
  Review, QnaQuestion, Instructor, Lecture,
} from "@/types";
...
export const authApi = {
  signup: (body: { name: string; nickname: string; email: string; password: string; role?: string }) =>
...
export const enrollmentApi = {
  ...
  cancel:         (courseId: number) =>
    request<ApiResponse<null>>(`/enrollments/${courseId}`, { method: "DELETE" }),
};
...
export const progressApi = {
  ...
  myProgress: () => request<{ success: boolean; data: ProgressItem[] }>("/progress"),
};

// ── Reviews ───────────────────────────────────────────────────
export const reviewApi = {
  list:   (courseId: number, page = 1) =>
    request<{ success: boolean; data: Review[]; pagination: { total: number; page: number; limit: number } }>(
  ...
  delete: (courseId: number, reviewId: number) =>
    request<ApiResponse<null>>(`/courses/${courseId}/reviews/${reviewId}`, { method: "DELETE" }),
};
...
export const qnaApi = {
  ...
  createAnswer: (courseId: number, questionId: number, content: string) =>
    request<ApiResponse<{ id: number; content: string; created_at: string; author_nickname: string; role: string }>>(
...
export const instructorApi = {
  ...
  detail: (id: number) => request<ApiResponse<Instructor & { courses: Course[] }>>(`/instructors/${id}`),
};
...
export const instructorCourseApi = {
  list: () =>
    request<ApiResponse<InstructorCourse[]>>("/instructor/courses"),

  create: (body: { title: string; description: string; level: string; price: number; is_live: boolean; duration_weeks?: number; thumbnail_url?: string }) =>
    request<ApiResponse<{ id: number }>>("/instructor/courses", { method: "POST", body: JSON.stringify(body) }),

  update: (courseId: number, body: object) =>
    request<ApiResponse<null>>(`/instructor/courses/${courseId}`, { method: "PUT", body: JSON.stringify(body) }),

  delete: (courseId: number) =>
    request<ApiResponse<null>>(`/instructor/courses/${courseId}`, { method: "DELETE" }),

  togglePublish: (courseId: number) =>
    request<ApiResponse<{ is_published: number }>>(`/instructor/courses/${courseId}/publish`, { method: "PATCH" }),
  ...
  addLecture: (courseId: number, body: LectureForm) =>
    request<ApiResponse<{ id: number }>>(`/instructor/courses/${courseId}/lectures`, { method: "POST", body: JSON.stringify(body) }),

  updateLecture: (courseId: number, lectureId: number, body: LectureForm) =>
    request<ApiResponse<null>>(`/instructor/courses/${courseId}/lectures/${lectureId}`, { method: "PUT", body: JSON.stringify(body) }),

  deleteLecture: (courseId: number, lectureId: number) =>
    request<ApiResponse<null>>(`/instructor/courses/${courseId}/lectures/${lectureId}`, { method: "DELETE" }),

  getProfile: () =>
    request<ApiResponse<{ name: string; bio: string; avatar_url: string | null }>>("/instructor/courses/profile"),

  updateProfile: (body: { name: string; bio: string; avatar_url: string }) =>
    request<ApiResponse<null>>("/instructor/courses/profile", { method: "PUT", body: JSON.stringify(body) }),
};
...
export const paymentApi = {
  prepare: (courseId: number) =>
    request<ApiResponse<PaymentOrder>>(`/payment/prepare/${courseId}`, { method: "POST" }),

  confirm: (body: { paymentKey: string; orderId: string; amount: number }) =>
    request<ApiResponse<{ paymentKey: string; orderId: string; amount: number; method: string }>>(
  ...
  myPayments: () =>
    request<ApiResponse<{ order_id: string; amount: number; status: string; paid_at: string; course_title: string; course_id: number }[]>>("/payment"),
};
